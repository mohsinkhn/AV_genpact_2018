...
def get_feats1(tr, val, log=True, target_col="num_orders"):
...
def main():
    ...
    tr1.to_csv(str(config.SAVE_PATH / "tr1f2.csv"), index=False)
    val1.to_csv(str(config.SAVE_PATH / "val1f2.csv"), index=False)

    tr2, val2 = get_feats1(tr2, val2)
    tr2.to_csv(str(config.SAVE_PATH / "tr2f2.csv"), index=False)
    val2.to_csv(str(config.SAVE_PATH / "val2f2.csv"), index=False)

    tr3, val3 = get_feats1(tr3, val3)
    tr3.to_csv(str(config.SAVE_PATH / "tr3f2.csv"), index=False)
    val3.to_csv(str(config.SAVE_PATH / "val3f2.csv"), index=False)

    train, test = get_feats1(train, test)
    train.to_csv(str(config.SAVE_PATH / "trainf2.csv"), index=False)
    test.to_csv(str(config.SAVE_PATH / "testf2.csv"), index=False)
